...
def pdf_processing_page():
    ...
    # Description
    st.markdown("**Note:** Do not over use the options 'Nanonet' and 'Adobe' as they are API based and may hit the max limit on Ambit server.")
    ...
    if pdf_file:
        ...
        if processing_method == "Camelot Stream":
            ...
            processed_data = process_with_camelot(pdf_file, int(page_number), flavor = 'stream')
        elif processing_method == "Camelot Lattice":
            page_number = st.number_input("Enter the page number to process:", min_value=1, value=1)
            processed_data = process_with_camelot(pdf_file, int(page_number), flavor = 'lattice')
        elif processing_method == "Tabula":
            page_number = st.number_input("Enter the page number to process:", min_value=1, value=1)
            processed_data = process_with_tabula(pdf_file, int(page_number))
...
def main():
    ...
    if pdf_file:
        ...
        if processing_method == "Camelot Stream":
            st.markdown("Camelot library scans PDFs and extracts tables automatically. You can choose between two table parsing methods/flavors, Stream and Lattice. Stream works well for tables with spaces between cells, like in regular text. However, this library only works with text-based PDFs, not scanned ones.")
            st.markdown("Choose a page number from your PDF to convert it to excel data")
        elif processing_method == "Camelot Lattice":
            st.markdown("Lattice method is great for tables with clear lines between cells. Plus, it can handle multiple tables on one page without guessing.")
            st.markdown("Choose a page number from your PDF to convert it to excel data")
        elif processing_method == "Tabula":
            st.markdown("Choose a page number from your PDF to convert all tables on it to excel data using Tabula library.")
        elif processing_method == "Nanonet":
            st.markdown("Nanonets utilizes an API that leverages image processing for PDF-to-Excel conversion. You can add and process one page at a time, providing flexibility and precision in extracting data from PDFs. This approach ensures efficient and accurate conversions for various document types.")
        elif processing_method == "Adobe":
            st.markdown("You can specify a range of page numbers. This feature ensures efficient processing of smaller PDFs, as Adobe's API imposes a limit of 500 pages in total.")
        ...
        try:
            if processed_data:
                ...
                for table_df in processed_data:
...
def process_with_camelot(pdf_file, page_number, flavor):
    # Save the uploaded PDF to a temporary file
    temp_pdf_path = "temp.pdf"
    with open(temp_pdf_path, "wb") as temp_pdf:
        temp_pdf.write(pdf_file.read())
    
    # Read the specified page of the PDF using Camelot with the selected flavor
    tables = camelot.read_pdf(temp_pdf_path, pages=str(page_number), flavor=flavor)
...
def process_with_tabula(pdf_file, page_number):
    # Save the uploaded PDF to a temporary file
    temp_pdf_path = "temp.pdf"
    with open(temp_pdf_path, "wb") as temp_pdf:
        temp_pdf.write(pdf_file.read())
    ...
    tables = tabula.read_pdf(temp_pdf_path, pages=str(page_number), multiple_tables=True)
...
# # Get and print the OpenCV version
